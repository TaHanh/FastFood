    ...
    return (
      <div className="profile-address w-100">
        <div className="">
          ...
          {this.isRender ? this.user.address && this.user.address.length > 0 ? (
        ...
        <div className="col-3 text-right">
         <button type="button" class="btn btn-outline-primary">Sửa</button>
              ...
              {this.user.address.type == '1' ? <button disabled type="button" class="btn btn-outline-dark">Thiết lập mặc định</button> :<button type="button" class="btn btn-outline-dark">Thiết lập mặc định</button> }
        </div>
        <div className="col-9">
                <div className="row">
                  ...
                  <div className="col-8">
                   {this.user.address.name} {this.user.address.type == '1' ? <span class="badge badge-pill badge-success">Mặc dịnh</span> : null }
                  </div>
                </div>
                <div className="row">
          ...
          ) : <div>Bạn chưa có địa chỉ nhận hàng nào ! Hãy thêm địa chỉ mới để mua hàng được thuận lợi!</div>
           : (
            <LoadComponent />
          )}
        </div>
